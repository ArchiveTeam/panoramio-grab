    ...
    if string.match(url, "/"..item_value.."[0-9][0-9]")
      ...
      for customurl in string.gmatch(html, '"(http[s]?://[^"]+)"') do
        if string.match(customurl, "/"..item_value.."[0-9][0-9]")
          or string.match(customurl, "="..item_value.."[0-9][0-9]")
          or string.match(customurl, "%?"..item_value.."[0-9][0-9]")
          or string.match(customurl, "[^0-9]"..item_value.."[0-9][0-9]")
...
--          end
        if string.match(customurlnf, "/"..item_value.."[0-9][0-9]")
          or string.match(customurlnf, "="..item_value.."[0-9][0-9]")
          or string.match(customurlnf, "%?"..item_value.."[0-9][0-9]")
          or string.match(customurlnf, "[^0-9]"..item_value.."[0-9][0-9]")
...
wget.callbacks.httploop_result = function(url, err, http_stat)
  ...
  elseif status_code == 400 and string.match(url["url"], "panoramio%.com/offensive/photo") then
    return wget.actions.EXIT
